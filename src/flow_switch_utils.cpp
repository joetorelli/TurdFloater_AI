#include "flow_switch_utils.h"

Flow_Switch_Utils::Flow_Switch_Utils(uint8_t flow_switch_pin)
{
	pinMode(flow_switch_pin, INPUT_PULLUP);
	_flow_switch_pin = flow_switch_pin;
}

bool Flow_Switch_Utils::isFlowing()
{
	return digitalRead(_flow_switch_pin) == LOW;
}
